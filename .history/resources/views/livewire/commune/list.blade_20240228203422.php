            ...
            <div class="card-body table-responsive p-0 table-striped" style="height: 300px;">
                <table class="table table-head-fixed">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $com->nom }}</td>
                            <td class="text-center">{{ optional($com->created_at)->diffForHumans() }}</td>
                            <td class="text-center">
                                <button class="btn btn-link" wire:click="editTypeArticle({{$com->id}})"> <i
                                        class="far fa-edit"></i> </button>

                                <button class="btn btn-link" wire:click="showProp({{$com->id}})"> <i
                                        class="fa fa-list"></i> propriétés</button>

                                @if(count($com->articles) == 0)
                                <button class="btn btn-link"
                                    wire:click="confirmDelete('{{$com->nom}}', {{$com->id}})"> <i
