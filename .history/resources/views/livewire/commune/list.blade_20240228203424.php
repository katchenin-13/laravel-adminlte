                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button class="btn btn-link" wire:click="addNewTypeArticle"> <i class="fa fa-check"></i>
                                    Valider</button>
                        ...
                        @endif
                        @foreach ($comnues as $comnue)
                        <tr>
                            <td>{{ $comnue->nom }}</td>
                            <td class="text-center">{{ optional($comnue->created_at)->diffForHumans() }}</td>
                            <td class="text-center">
                                <button class="btn btn-link" wire:click="editTypeArticle({{$comnue->id}})"> <i
                                        class="far fa-edit"></i> </button>

                                <button class="btn btn-link" wire:click="showProp({{$comnue->id}})"> <i
                                        class="fa fa-list"></i> propriétés</button>

                                @if(count($comnue->articles) == 0)
                                <button class="btn btn-link"
                                    wire:click="confirmDelete('{{$comnue->nom}}', {{$comnue->id}})"> <i
